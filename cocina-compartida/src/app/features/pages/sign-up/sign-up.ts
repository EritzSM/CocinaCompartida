...
export class SignUp {
  ...
  avatarPreview = this.DEFAULT_AVATAR;
  showPassword = false;
  ...
  removeAvatar(): void {
    ...
    this.signUpForm.patchValue({ avatar: this.DEFAULT_AVATAR });
  ...
  async onSignUp(): Promise<void> {
    ...
    try {
      const res: LoginResponse = await this.authService.signup(userData);
      if (!res.success) {
        this.showAlert('Ops!', res.message || 'Error en el registro. Por favor intente nuevamente.');
        return;
      }
